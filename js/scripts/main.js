...
function createCardPokemon(code, types, imagePok, nome){
 let card = document.createElement('button');
 card.classList = `card-pokemon ${types} js-open-details-pokemon `;
 ...
 let tipoPokemon = document.createElement('div');
 tipoPokemon.classList = 'icon';
 infoCardPokemon.appendChild(tipoPokemon);

 let imgTipoPokemon = document.createElement('img');
 imgTipoPokemon.setAttribute('src', `img/icon-types/${types}.svg`);
 tipoPokemon.appendChild(imgTipoPokemon);
  ...
  .then((response)=>{
    ...
    results.forEach( pokemon =>{
      ...
      .then( response => {
        ...
        createCardPokemon(infoCard.code, infoCard.type, infoCard.image, infoCard.nome );
...
function openDetailsPokemon(){
  ...
  let nameTypePokemon = this.querySelector('.info .text h3');
  let codeTypePokemon = this.querySelector('.info .text span');
  ...
  namePokemonModal.textContent = nameTypePokemon.textContent;
  codePokemonModal.textContent = codeTypePokemon.textContent;
  ...
  function listingWeaknesses(){
   ...
   .then(response => {
     ...
     weaknessess.forEach(itemType => {
      ...
      spanListWeak.textContent = primeiraLetraMaiuscula(itemType.name);
...
function filterByTypes() {
 ...
 const sectionPokemons = document.querySelector('.s-all-info-pokemon');
...
function searchPokemon() {
 ...
 .catch((error) => {
   if(error.response){
    ...
    alert('Não foi encontrado nenhum resultado para a sua busca');
   }
 })
